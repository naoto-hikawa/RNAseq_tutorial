...
write.table(resOrdered, file=paste("deg_results/", stats, sep=""), sep="\t", append=F, quote=F, row.names=F)
...
write.table(resSIG_FC, file=paste("deg_results/", sig_FC, sep=""), sep="\t", append=F, quote=F, row.names=F)

# Write DEGs with Fold Change > 1.5
sig_FC_up=paste(name, "_DEGs_FC_up.txt", sep="")
write.table(resSIG_FC_up, file=paste("deg_results/", sig_FC_up, sep=""), sep="\t", append=F, quote=F, row.names=F)

# Write DEGs with inverse Fold Change > 1.5
sig_FC_down=paste(name, "_DEGs_FC_down.txt", sep="")
write.table(resSIG_FC_down, file=paste("deg_results/", sig_FC_down, sep=""), sep="\t", append=F, quote=F, row.names=F)
...
write.table(DESeq2_normalize, file=paste("deg_results/", normalized_file, sep=""), sep="\t", append=F, quote=F, row.names=F) 
...
write.table(GSEASig[1:10], file=paste("deg_results/", GSEA_DEG, sep=""), sep="\t", append=F, quote=F, row.names=F)
